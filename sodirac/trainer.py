...
class train_annotate(train_integrate):
    ...
    def _train_novel(
        ...
        for epoch in pbar:
            ...
            for iter_id, (graph_source, graph_target) in enumerate(zip(graph_dl_source, graph_dl_target)):
                ...
                # p_l_model = ExponentialMovingAverage((nclass,), init_value=1 / nclass).to(self.device)
                # p_labeled = p_l_model(pseudolabels_source.mean(dim=0))
                # p_u_model = MovingAverage((nclass,), buffer_size = round(pseudolabels_target.shape[0]/nclass), init_value=1 / nclass).to(self.device)
                # p_unlabled = p_u_model(pseudolabels_target.mean(dim=0)) 
                # expectation_ratio = (1e-6 + p_labeled) / (1e-6 + p_unlabled) ##### 
            ...
            if epoch > epochs // 4 * 3:
                if test_epoch_accuracy > best_acc:
                    torch.save({'minemodel_weights': self.minemodel.state_dict(),
    ...
    def _train(
        ...
        for epoch in pbar:
            ...
            for iter_id, (graph_source, graph_target) in enumerate(zip(graph_dl_source, graph_dl_target)):
                ...
                ######## calculate reconstruction loss
                loss_re = criterion_re(torch.cat([source_data, target_data], dim = 0), torch.cat([recon_feats[0], recon_feats[1]], dim = 0))
                # loss_re = F.cosine_similarity(torch.cat([source_data, target_data], dim = 0), torch.cat([recon_feats[0], recon_feats[1]], dim = 0), dim=0).mean()
                ###### Across privates domain loss
        ...
        best = torch.load(os.path.join(self.save_path, "Dirac_annotation.pt"), map_location=torch.device('cpu'))
        self.minemodel.load_state_dict(best['minemodel_weights'])
        self.minemodel.to(self.device)
        return self.minemodel
    ...
    def evaluate_target(
        ...
        if return_lists_roc:
            return feats_numpy, output, pred 
        return pred
